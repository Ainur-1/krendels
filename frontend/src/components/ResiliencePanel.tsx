...
function PlacementSection({ report }: { report: PlacementReport }) {
  ...
  // Ровная поверхность — это тоже ответ, и молчать о ней нельзя: одинаково зелёная
  // сетка иначе читается как «везде отлично», а означает «место ничего не решает».
  const flat = best - Math.min(...gains) < 0.5;
  ...
  return (
    <div>
      ...
      <p className="hint" style={{ marginTop: 0 }}>
        ...
        {flat
          ? " Поверхность ровная: разница между лучшим и худшим местом меньше половины пункта, то есть место не решает ничего. Так бывает, когда межспутниковая сеть цела и довозит трафик куда угодно."
          : " Чем темнее клетка, тем больше прирост. Неровная поверхность означает, что сеть не довозит трафик сама и шлюз должен стоять ближе к пунктам."}
      </p>
