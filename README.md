...
### ▶ Сервис развёрнут и открыт: **http://161.104.53.181:8000/**

Ничего устанавливать не нужно — откройте ссылку и нажмите «Рассчитать». Инструкция по работе с сервисом встроена в интерфейс: кнопка **«Инструкция»** в верхней строке. Загрузить свой сценарий можно там же, кнопкой «Загрузить файл».
...
Интерфейс — на `http://localhost:8000`, схема API — на `/docs` (у развёрнутого сервиса — [/docs](http://161.104.53.181:8000/docs)). Если фронтенд не собран, сервис работает и говорит об этом на корневой странице, а не отвечает пустым 404.
...
**Расчётное ядро отделено от интерфейса.** Пакет `cosmo_net` ничего не знает про HTTP и про React; сервис и скрипты — тонкие обёртки над ним.

**Геометрия считается таблицами, и это не про скорость.** Весь горизонт — один массив `(720, 48, 3)`, циклов в расчёте нет. Мы написали ту же геометрию обычными циклами и замерили: таблицы быстрее всего **в 1.4 раза** (0.12 с против 0.17 с при полностью совпадающих числах). На сорока восьми аппаратах время уходит не на арифметику, а на выделение памяти. Ценность в другом: весь горизонт — одно выражение, ошибиться в индексах негде, а из таблиц дёшево получаются прореженная сетка для перебора и подмена маски «кто в строю» без пересчёта положений.
...
**Настоящие ускорения — алгоритмические, и они измерены.** Доступность — вопрос связности, а не поиска маршрута, поэтому она считается объединением множеств: 38 мс против 99 мс у полного прогона с маршрутизацией, выигрыш 2.6 раза. Положения не зависят от того, кто в строю, поэтому выбивание 48 аппаратов по очереди переиспользует геометрию и меняет только маски: 1.65 с против 3.4 с, выигрыш вдвое. Перебор конфигураций идёт в две стадии — сотня кандидатов ранжируется на прореженной сетке, точно перемеряется дюжина лучших.

Порядок величин: прогон суток — около 100 мс, критичность 48 аппаратов — под две секунды, перебор 101 конфигурации — около двух секунд на восьми процессах. Поэтому подбор конфигурации живёт за кнопкой, а не за прогресс-баром. Это замеры на одной машине, и на другой они будут другими — важны отношения, а не абсолютные числа.

## Что сервис умеет

- Встроенная инструкция «как пользоваться сервисом» — кнопка в верхней строке.
...
- Выгрузка сценария в формате `cosmo-A-1.0` — со всеми правками, загружается обратно.
- Выгрузка результата в формате `cosmo-A-result-1.0`; файл результата тоже принимается в поле сценария.
...
**Число одновременных межспутниковых линий не ограничено.** Связь существует, если аппараты ближе предела дальности и между ними не Земля; сколько лучей аппарат способен вести разом, кейс не задаёт, и мы этого не ограничиваем. Измерено, чего это стоит: в среднем у аппарата 3.09 связи, максимум 7, и на 25.3 % отсчётов их больше четырёх. Реальный аппарат обычно несёт четыре терминала — два вдоль плоскости и два поперёк, — поэтому часть одновременных связей в нашей модели в железе не существовала бы. Практический след этого допущения виден в сценарии 03, где отказы рвут короткие переходы и маршрут уходит вокруг планеты: самая длинная трасса там — 18 переходов и 45 827 км, то есть 306 мс туда-обратно против обычных 27 мс. Путь допустимый, но система, у которой есть требование по задержке, такой трафик отбросила бы, а не повезла.
