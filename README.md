...
Эталонный `validate()` останавливается на первой ошибке; здесь файл с тремя ошибками даёт три, и интерфейс подсвечивает поля.
...
reports/metrics/ измеренные показатели, на которые ссылается документация
